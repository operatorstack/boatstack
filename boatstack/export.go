...
func BuildExportBundle(configPath string, config ProjectConfig, rawConfig []byte, adapterName string) (ExportBundle, error) {
	...
	operations["plan-gate"] += " check-plan must return current READINESS_FINGERPRINT before approval is displayed. Stop on any branch, worktree, origin, base, upstream, or journey-capability block."
...
Ordinary product intent starts in the host's Plan mode. Pass the resulting plan to auto-plan explicitly with --plan <path>; Boatstack never scans directories for plans, and must not invent a substitute. Keep that plan file present and current through build.
