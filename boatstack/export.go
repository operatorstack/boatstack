...
func BuildExportBundle(configPath string, config ProjectConfig, rawConfig []byte, adapterName string) (ExportBundle, error) {
	...
	operations["boatstack-run"] += " If status is NOT_STARTED, route to auto-plan; planning and plan-gate do not require delivery preflight. If Cursor reports MainThreadShellExec not initialized, make Developer: Reload Window the one recovery action."
	...
	operations["test-gate"] += " For relevant journey evidence, run every compiled oracle and import current typed results with record-journey-results before the gate."
	operations["review-gate"] += " Reject missing, failed, manifest-mismatched, head-mismatched, or diff-stale required journey results."
	operations["ship-gate"] += " When a current autonomy.md has target pr, its repository, branch, plan, and open/update action match, and the exact preview fingerprint still passes check-pr, invoke publish-pr with --autonomy <autonomy.md> without asking for o or u. Any mismatch invalidates autonomous publication and returns to the operator; runs without that receipt retain the normal confirmation."
