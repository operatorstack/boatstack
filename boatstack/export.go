...
func commandBody(operation, extra string) string {
	...
	if operation == "auto-plan" {
		preflight = `Before reading repository context or drafting artifacts, identify the durable in-repo path of the plan produced in the active host/system conversation — the user supplies it as ` + "`/auto-plan <plan-file>`" + `. Boatstack does not scan directories for plans. If no path is available, stop and ask for it; do not create or guess a substitute. For every planning artifact, send its complete Markdown to the read-only ` + "`flow bootstrap --repo . --feature <slug> --source-plan <host-path> --artifact <name> --shell posix|powershell`" + ` oracle through the current Boatstack operation entrypoint. Execute only the returned ` + "`planning_envelope`" + `, unchanged. Never construct a helper path or planning-write command from documentation. Re-resolve after every workspace cut from its ` + "`destination_repository`" + `.`
	}
