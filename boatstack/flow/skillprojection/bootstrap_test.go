...
func TestBootstrapContractFailsClosedBeforeFlowExecution(t *testing.T) {
	...
	for _, expected := range []string{
		...
		"BOATSTACK_EXPECTED_RUNTIME_SHA256=<exact-sha256>", "/<exact-version>/install.sh",
	} {
	...
	if strings.Contains(contract, "v9.9.10") {
		t.Fatal("bootstrap contract depends on the generating runtime version")
	}
}
