...
func TestChangedFacetsPreservesExactDomainValues(t *testing.T) {
	...
	before.Goal = model.Goal{ID: "goal-a", Kind: model.GoalOpenPR, DeliveryID: "delivery-a"}
	after := before
	...
	if after.ProgramFingerprint != before.ProgramFingerprint || after.Goal != before.Goal {
