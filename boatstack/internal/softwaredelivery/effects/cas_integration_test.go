...
func TestConcurrentApplyConsumesOneRevisionExactlyOnce(t *testing.T) {
	...
	program := testProgram()
	kernelA, err := boatstack.NewDeliveryController(externalRoot, program)
	if err != nil {
		t.Fatal(err)
	}
	kernelB, err := boatstack.NewDeliveryController(externalRoot, program)
	if err != nil {
		t.Fatal(err)
	}
	...
	for _, kernel := range []boatstack.DeliveryController{kernelA, kernelA, kernelB} {
		kernel := kernel
		go func() {
			<-start
...
func TestProgramChangeInvalidatesPriorPrescriptionBeforeEffects(t *testing.T) {
	...
	programQ, err := delivery.Compile(ctx, delivery.CompileRequest{
	...
	kernelP, _ := boatstack.NewDeliveryController(externalRoot, programP)
	kernelQ, _ := boatstack.NewDeliveryController(externalRoot, programQ)
	executable, _ := os.Executable()
	executable, _ = filepath.Abs(executable)
	executable, _ = filepath.EvalSymlinks(executable)
	runtimeRaw, _ := os.ReadFile(executable)
