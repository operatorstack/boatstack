...
var nativeStateHandlers = map[string]nativeStateHandler{
	...
	"work-package-approve":           applyWorkPackageApprove,
...
func resetDeliveryState(state *durable.State) {
	state.Delivery, state.WorkPackage, state.Plan = model.DeliveryUninitialized, model.WorkPackageAbsent, model.PlanAbsent
	...
	state.PlanFingerprint, state.WorkPackageFingerprint, state.WorkPackageApprovalFingerprint, state.ApprovalFingerprint, state.PublicationID, state.PublicationURL, state.PreviewFingerprint = "", "", "", "", "", "", ""
...
func applyWorkPackageAdmit(state *durable.State, _ protocol.Admission, _ catalog.Transition) error {
	state.WorkPackage, state.Delivery, state.Phase, state.Terminal = model.WorkPackageValid, model.DeliveryPlanning, model.PhaseActive, model.TerminalNonterminal
	return nil
}

func applyWorkPackageApprove(state *durable.State, admission protocol.Admission, _ catalog.Transition) error {
	state.WorkPackage, state.Delivery, state.Phase = model.WorkPackageApproved, model.DeliveryApproved, model.PhaseActive
	if admission.Objective.TrustedObjectiveClass() == model.ObjectiveApprovedWorkPackage {
		establishTerminal(state, model.PhaseTerminal)
	}
	return nil
}
