...
func testObservation(phase ProtocolPhase) Observation {
	...
	return Observation{
		...
		Terminal: Known(TerminalNonterminal, evidence), Objective: Absent[Objective]("not configured", evidence), ObservedAt: time.Unix(100, 0).UTC(),
	}
}
...
func TestCanonicalizeRejectsEstablishedTerminalInActivePhase(t *testing.T) {
	// control-law: terminal-is-an-evidence-backed-objective-state
