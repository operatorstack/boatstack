...
func renderCatalogLocus(transitions []catalog.Transition, safety bool) (string, error) {
	...
	result := locusModel{
		SchemaVersion: 1,
		ID:            "boatstack-executable-catalog-liveness-v1",
		...
		Evidence: []locusEvidence{
			{Path: "boatstack/delivery/delivery.go", Note: "Compiler combines exact CoreSystem, ProgramRuntime, extension, contract, and ownership declarations into one immutable runtime registry."},
			{Path: "docs/architecture/boatstack-transition-catalog.md", Note: "Generated readable projection from the same runtime registry."},
			{Path: "boatstack/internal/softwaredelivery/protocol/admission.go", Note: "Exact admission, authority, parameter, source-revision, provider-request, expiry, and stale-snapshot checks."},
			{Path: "boatstack/internal/softwaredelivery/engine/engine.go", Note: "Single apply path across lock, journal, effect, fresh observation, target predicate, receipt, and recovery."},
			{Path: "boatstack/internal/softwaredelivery/effects/state_reducer.go", Note: "Admitted native effects reduce every controllable Standard distribution transition through one state adapter."},
			{Path: "boatstack/flow/standard/completeness_test.go", Note: "Runtime facet/event classification, writer-boundary inventory, and reducer-completeness refusing tests."},
			{Path: "boatstack/internal/softwaredelivery/engine/engine_test.go", Note: "Exact-admission, stale-snapshot, postcondition, interruption, idempotency, and unknown-outcome tests."},
			{Path: "boatstack/internal/softwaredelivery/effects/prepared.go", Note: "Staged effect ordering, atomic resource application, rollback, and external settlement boundary."},
			{Path: "boatstack/flow/standard/historical_test.go", Note: "Historical incidents resolved through the executable runtime supervisor."},
		},
		Spec: locusSpec{
	...
	if safety {
		result.ID = "boatstack-executable-catalog-safety-v1"
