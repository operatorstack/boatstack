...
func renderCatalogLocus(transitions []catalog.Transition, safety bool) (string, error) {
	...
	result := locusModel{
		...
		Evidence: []locusEvidence{
			...
			{Path: "boatstack/internal/effects/state_reducer.go", Note: "Admitted native effects reduce every controllable Standard distribution transition through one state adapter."},
