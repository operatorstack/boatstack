...
That is why context trimming is not automatically an optimization. If removing state increases rework or false acceptance, total cost rises. The canonical runtime references are approximately **10474 estimated tokens**, while host adapters point to one operation at a time.
