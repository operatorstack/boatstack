...
The repository also declares `identity.human` in `.boatstack/project.json`.
Boatstack exposes that literal or structured command descriptor and its
fingerprint to the host; it does not execute the command. The host resolves and
